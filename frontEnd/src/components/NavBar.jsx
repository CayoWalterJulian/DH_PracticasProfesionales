...
const NavBar = () => {
  ...
  return (
    <nav className="bg-blue-900 p-4 flex items-center justify-between z-50 relative">
      ...
      {/* Menú */}
      <ul className={`fixed top-0 left-0 h-screen w-full bg-blue-900 flex flex-col items-center justify-center transform transition-transform duration-300 ease-in-out ${menuOpen ? 'translate-x-0' : 'translate-x-full'} md:static md:flex md:flex-row md:h-auto md:w-auto z-40`}>
        <li className="text-white py-2 md:py-0 md:px-4 cursor-pointer" onClick={toggleMenu}>
          <Link to="/search-talent">Buscar talento</Link>
        </li>
        <li className="text-white py-2 md:py-0 md:px-4 cursor-pointer" onClick={toggleMenu}>
          <Link to="/formulario">Postularse</Link>
        </li>
        <li className="text-white py-2 md:py-0 md:px-4 cursor-pointer" onClick={toggleMenu}>
